...
class SEPW_Widget_Config extends WP_Widget
{
	...
	function SEPW_Widget_Config()
	{
		...
		$this->se_front_sites = SEPW_Widget_Print::filter_meta_sites( $this->se_sites );
		$this->order = array(
		...
		$widget_ops = array(
			...
			'description'	 => __( "Display posts by User in any Stack Exchange site. Or show the site's recent activity.", 'sepw' ) 
		);
		$control_ops = array(
			'width'		 => 300,
			'height'	 => 350,
	...
	function widget_style()
	{
		...
		if( !isset( $opts['css'] ) )
			wp_enqueue_style( 
					'se-widget', 
					SEPW_Widget_Init::get_instance()->plugin_url . 'css/sepw.css' 
			);
	}
	...
	{
		extract( $args );
        $option = get_option( SEPW_Widget_Init::$option_name );
        if( isset( $option['disable_meta'] ) )
            $rand_sites = $this->se_front_sites;
        else
            $rand_sites = $this->se_sites;
        
		$title = apply_filters( 'widget_title', $instance['title'] );
		$stack_site = ( isset( $instance['random'] ) ) ? array_rand( $rand_sites, 1 ) : $instance['stack_site'];
		...
		//( $build, $get_se, $user, $before_title, $after_title, $title, $stack_site, $q_or_a, $subtitle, $se_sites )
		if( !empty( $instance['user'] ) )
			$build_widget .= SEPW_Widget_Print::print_user( array(
					'build' => $build_widget, 
					'get_se' => $get_se, 
					'user' => $instance['user'], 
					...
					'q_or_a' => $q_or_a, 
					'subtitle' => $subtitle, 
					'se_sites' => $this->se_sites 
			));
		else 
			$build_widget .= SEPW_Widget_Print::print_site( array(
					'build' => $build_widget, 
					'get_se' => $get_se, 
					'before_title' => $before_title, 
					'after_title' => $after_title, 
